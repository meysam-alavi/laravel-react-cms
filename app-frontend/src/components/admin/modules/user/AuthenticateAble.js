 ...
 */
class AuthenticateAble extends BaseComponent {
    ...
    login(frmData) {
        ...
        axiosInstance.post(url, frmData).then(response => {
            ...
            if (result.success === true && result.data.token.length) {
                ...
                window.location = `/${this.getLang()}/admin/module/expenses-manager/expenses-listing`;
            } else {
                const messages = {
                    errors: {
    ...
    unauthenticated() {
        ...
        window.location = `/${this.getLang()}/admin/login`;
    }
